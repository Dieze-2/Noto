              ...
              {exercises.map((ex) => {
                ...
                return (
                  <motion.div
                    ...
                    className="relative"
                  >
                    {/* Fond rouge progressif au swipe */}
                    <motion.div 
                      style={{ opacity: bgOpacity }}
                      className="absolute inset-0 bg-rose-600 rounded-[1.5rem]"
                    />
                    ...
                    >
                      <GlassCard className="p-4 flex justify-between items-center border-white/5">
                        ...
                        <button onClick={() => handleDelete(ex.id)} className="p-2 text-white/10 hover:text-rose-500 transition-colors">
                          <Trash2 size={18} />
                        </button>
                      </GlassCard>
                    </motion.div>
                ...
                <div className="grid grid-cols-3 gap-3">
                   <div className="flex bg-white/5 rounded-xl p-1 h-11">
                    {(["KG", "PDC_PLUS"] as const).map(type => (
                      <button key={type} className={`flex-1 rounded-lg font-black text-[9px] uppercase ${newEx.load_type === type ? 'bg-menthe text-black' : 'text-white/30'}`} onClick={() => setNewEx({...newEx, load_type: type})}>
                        {type === "PDC_PLUS" ? "PDC+" : "KG"}
                      </button>
                    ))}
                  </div>
                  <input type="number" placeholder="kg" className="bg-white/5 border border-white/10 rounded-xl px-2 text-center font-black outline-none" value={newEx.weight} onChange={(e) => setNewEx({...newEx, weight: e.target.value})} />
                  <input type="number" placeholder="reps" className="bg-white/5 border border-white/10 rounded-xl px-2 text-center font-black outline-none" value={newEx.reps} onChange={(e) => setNewEx({...newEx, reps: e.target.value})} />
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setShowAddForm(false)} className="flex-1 py-3 bg-white/5 rounded-xl font-black uppercase text-xs">Annuler</button>
                  <button onClick={onAdd} className="flex-1 py-3 bg-menthe rounded-xl font-black uppercase text-xs text-black">Valider</button>
                </div>
              </GlassCard>
            ) : (
              <button onClick={() => setShowAddForm(true)} className="w-full py-6 border-2 border-dashed border-white/5 rounded-[2rem] flex flex-col items-center justify-center text-white/20">
                <Plus size={24} />
