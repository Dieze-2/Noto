...
export default function TodayPage() {
  ...
  const formatDisplayLoad = (type: string, grams: number | null) => {
    ...
    if (type === "PDC_PLUS") {
      ...
      return Number.isInteger(kg) ? `PDC + ${kg}` : `PDC + ${kg.toString().replace('.', ',')}`;
    }
    ...
    <div className="max-w-xl mx-auto px-4 pt-4 pb-32 space-y-6">
      ...
      <section className="glass-card p-6 rounded-[2.5rem] space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1">
            <label className="text-[9px] font-black uppercase pl-2 opacity-40">Poids (kg)</label>
            <input type="number" step="0.1" value={metrics.weight} onChange={e => setMetrics({...metrics, weight: e.target.value})} onBlur={handleSaveMetrics} className="w-full bg-white/5 p-4 rounded-2xl font-black text-white outline-none focus:ring-1 ring-menthe" />
          </div>
          <div className="space-y-1">
            <label className="text-[9px] font-black uppercase pl-2 opacity-40">Pas</label>
            <input type="number" value={metrics.steps} onChange={e => setMetrics({...metrics, steps: e.target.value})} onBlur={handleSaveMetrics} className="w-full bg-white/5 p-4 rounded-2xl font-black text-white outline-none focus:ring-1 ring-menthe" />
          </div>
          <div className="space-y-1">
            <label className="text-[9px] font-black uppercase pl-2 opacity-40">Calories</label>
            <input type="number" value={metrics.kcal} onChange={e => setMetrics({...metrics, kcal: e.target.value})} onBlur={handleSaveMetrics} className="w-full bg-white/5 p-4 rounded-2xl font-black text-white outline-none focus:ring-1 ring-menthe" />
          </div>
        </div>
      </section>
      ...
      <section className="glass-card p-6 rounded-[2.5rem] space-y-4 border-b-4 border-menthe">
        <div className="relative">
          <input 
            ...
            onChange={async (e) => {
              ...
              if(e.target.value.length > 1) {
                const list = await listCatalogExercises(e.target.value);
                setSuggestions(list);
              } else setSuggestions([]);
            }}
          ...
          {suggestions.length > 0 && (
            <div className="absolute z-10 w-full mt-2 bg-[#1a1a1a] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
              {suggestions.map(s => (
                <button key={s.id} onClick={() => { setNewName(s.name); setSuggestions([]); }} className="w-full p-4 text-left text-sm font-bold border-b border-white/5 hover:bg-menthe hover:text-black transition-colors">{s.name}</button>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2">
          <select value={newLoadType} onChange={e => setNewLoadType(e.target.value as any)} className="bg-white/5 p-4 rounded-2xl font-bold text-xs text-white outline-none appearance-none">
            <option value="KG">KG</option>
            <option value="PDC">PDC</option>
            <option value="PDC_PLUS">PDC+</option>
          </select>
          {newLoadType !== "PDC" && (
            <input type="number" placeholder="Charge" value={newLoadVal} onChange={e => setNewLoadVal(e.target.value)} className="flex-1 bg-white/5 p-4 rounded-2xl font-bold text-white outline-none" />
          )}
          <input type="number" placeholder="Reps" value={newReps} onChange={e => setNewReps(e.target.value)} className="w-24 bg-white/5 p-4 rounded-2xl font-bold text-white outline-none" />
        </div>
        ...
        <button 
          onClick={async () => {
            ...
            await addWorkoutExercise({
              ...
              load_g: newLoadType !== "TEXT" ? kgToGramsInt(parseDecimalFlexible(newLoadVal) ?? 0) : null,
      ...
      <div className="space-y-3">
        {exercises.map(ex => (
          <div key={ex.id} className="relative group overflow-hidden rounded-3xl touch-action-pan-x">
            {/* BOUTON SUPPRIMER FIXÉ */}
            ...
            <div className="relative glass-card p-5 flex justify-between items-center transition-transform group-active:-translate-x-20 duration-300 bg-[#121212]">
              <div>
