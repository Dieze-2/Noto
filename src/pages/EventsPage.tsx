  ...
  return (
    <div className="max-w-xl mx-auto px-4 pt-8 pb-32 space-y-8">
      ...
      <section className="glass-card p-6 rounded-[2.5rem] space-y-4 border-b-4 border-menthe">
        ...
        <button onClick={async () => { if(!title) return; await createEvent({ title, start_date: from, end_date: to, color: '#00ffa3' }); setTitle(""); refresh(); }} className="w-full bg-menthe text-black py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Ajouter</button>
      </section>
