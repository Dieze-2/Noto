...
import { Shield, Crown, X, Loader2, Clock, UserCheck, UserX, Play, XCircle } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const [pending, pendingCancels] = await Promise.all([
      getPendingCoachRequests(),
    ...
    const enrichedRequests = await Promise.all(
      pending.map(async (req) => {
        ...
        return { ...req, profileName: profile ? displayName(profile) : req.user_id.slice(0, 8) };
      })
    );
                  ...
                  <GlassCard className="p-4 rounded-2xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground font-bold">{new Date(req.created_at).toLocaleDateString()}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleApprove(req.id)} disabled={!!actionId}
                          ...
                          {actionId === req.id ? <Loader2 size={12} className="animate-spin" /> : <UserCheck size={12} />}
                          {t("admin.approve")}
                        </button>
                        <button onClick={() => handleReject(req.id)} disabled={!!actionId}
